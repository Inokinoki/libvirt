/*
 * macosvfsuspendtest.c: test macOS Virtualization.Framework suspend/resume and
 * statistics
 *
 * Copyright (C) 2025
 *
...
static int testCPUStats(const void *data G_GNUC_UNUSED) {
...
static int testMemoryStats(const void *data G_GNUC_UNUSED) {
  ...
  /* Test memory stats (should return configured memory even when not running) */
  nstats = macosvfVMGetMemoryStats(vm, stats, 3);
  if (nstats < 0) {
    fprintf(stderr, "%s: Failed to get memory stats\n", __FUNCTION__);
    goto cleanup;
  }

  /* When VM is not running, AVAILABLE memory should still be configured memory */
  if (stats[0].tag == VIR_DOMAIN_MEMORY_STAT_AVAILABLE && stats[0].val != 1024 * 1024) {
    fprintf(stderr, "%s: Expected memory %llu for stopped VM, got %llu\n",
            __FUNCTION__, (unsigned long long)1024 * 1024, stats[0].val);
    goto cleanup;
  }

  /* ACTUAL_BALLOON should also be returned */
  if (nstats < 2 || stats[1].tag != VIR_DOMAIN_MEMORY_STAT_ACTUAL_BALLOON) {
    fprintf(stderr, "%s: Expected second stat to be ACTUAL_BALLOON\n", __FUNCTION__);
    goto cleanup;
  }

  /* RSS should not be returned for stopped VM (only 2 stats, not 3) */
  if (nstats != 2) {
    fprintf(stderr, "%s: Expected 2 stats for stopped VM (no RSS), got %d\n", __FUNCTION__, nstats);
    goto cleanup;
  }
...
static int testVMStateTransitions(const void *data G_GNUC_UNUSED) {
  virDomainDef *def = NULL;
  macosvfVMObject *vm = NULL;
...
static int testPauseResumeErrors(const void *data G_GNUC_UNUSED) {
  virDomainDef *def = NULL;
  macosvfVMObject *vm = NULL;
  int ret = -1;
...
static int testStatsMultiVCPU(const void *data G_GNUC_UNUSED) {
  virDomainDef *def = NULL;
  macosvfVMObject *vm = NULL;
  unsigned long long cpuTime = 0;
  int ret = -1;
...
static int testVMLifecycleStates(const void *data G_GNUC_UNUSED) {
  virDomainDef *def = NULL;
  macosvfVMObject *vm = NULL;
  macosvfVMState state;
  int ret = -1;
...
static int mymain(void) {
  ...
  /* Test CPU statistics */
  if (virTestRun("MACOSVF CPU Stats", testCPUStats, &info) < 0)
    ret = -1;

  /* Test memory statistics */
  if (virTestRun("MACOSVF Memory Stats", testMemoryStats, &info) < 0)
    ret = -1;

  /* Test VM state transitions */
  if (virTestRun("MACOSVF VM State Transitions", testVMStateTransitions,
                 &info) < 0)
    ret = -1;

  /* Test pause/resume error handling */
  if (virTestRun("MACOSVF Pause/Resume Errors", testPauseResumeErrors, &info) <
      0)
    ret = -1;

  /* Test statistics with multi-vCPU */
  if (virTestRun("MACOSVF Stats Multi-VCPU", testStatsMultiVCPU, &info) < 0)
    ret = -1;

  /* Test VM lifecycle states */
  if (virTestRun("MACOSVF VM Lifecycle States", testVMLifecycleStates, &info) <
      0)
    ret = -1;
