...
VIR_ENUM_IMPL(remoteDriverTransport, REMOTE_DRIVER_TRANSPORT_LAST, "tls",
              "unix", "ssh", "libssh2", "ext", "tcp", "libssh");

VIR_ENUM_IMPL(remoteDriverMode, REMOTE_DRIVER_MODE_LAST, "auto", "legacy",
              "direct");

#ifndef WIN32
static const char *remoteGetDaemonPathEnv(void) {
...
int remoteSplitURIScheme(virURI *uri, char **driver,
                         remoteDriverTransport *transport) {
  char *p = NULL;

  if (!uri->scheme) {
    virReportError(VIR_ERR_INVALID_ARG, "%s", _("missing scheme for URI"));
    return -1;
  }
  ...
  if (p) {
    ...
    if ((val = remoteDriverTransportTypeFromString(tmp)) < 0) {
      virReportError(VIR_ERR_INVALID_ARG, "%s",
                     _("remote_open: transport in URL not recognised (should "
                       "be tls|unix|ssh|ext|tcp|libssh2|libssh)"));
      return -1;
    }

    if (val == REMOTE_DRIVER_TRANSPORT_UNIX && uri->server) {
      virReportError(
          VIR_ERR_INVALID_ARG,
...
static char *remoteGetUNIXSocketHelper(remoteDriverTransport transport,
  ...
  if (flags & REMOTE_DRIVER_OPEN_USER) {
     ...
     */
    sockname =
        g_strdup_printf("%s/run/libvirt/%s-%s", LOCALSTATEDIR, sock_prefix,
                        flags & REMOTE_DRIVER_OPEN_RO ? "sock-ro" : "sock");
  }

  VIR_DEBUG("Built UNIX sockname=%s for transport=%s "
            "prefix=%s flags=0x%x",
            sockname, remoteDriverTransportTypeToString(transport), sock_prefix,
            flags);
  return sockname;
}
 ...
 */
int remoteProbeSessionDriverFromBinary(char **driver) {
  ...
  *driver = NULL;
  ...
  for (i = 0; i < (ssize_t)G_N_ELEMENTS(drivers); i++) {
    ...
    if (!(daemonpath = virFileFindResource(daemonname, abs_top_builddir "/src",
                                           SBINDIR)))
      return -1;
...
int remoteProbeSystemDriverFromSocket(bool readonly, char **driver) {
  /* Order these the same as virDriverLoadModule
   * calls in daemonInitialize, so we replicate
   * probing order that virConnectOpen would use
   * if running inside libvirtd */
  ...
  for (i = 0; i < (ssize_t)G_N_ELEMENTS(drivers); i++) {
    g_autofree char *sockname =
        g_strdup_printf("%s/libvirt/virt%sd-%s", RUNSTATEDIR, drivers[i],
                        readonly ? "sock-ro" : "sock");
...
int remoteProbeSessionDriverFromSocket(bool readonly, char **driver) {
  /* Order these the same as virDriverLoadModule
  ...
  for (i = 0; i < (ssize_t)G_N_ELEMENTS(drivers); i++) {
    ...
    g_autofree char *sockname = g_strdup_printf(
        "%s/virt%sd-%s", userdir, drivers[i], readonly ? "sock-ro" : "sock");
...
char *remoteGetUNIXSocket(remoteDriverTransport transport,
                          remoteDriverMode mode, const char *driver,
                          unsigned int flags, char **daemon_path) {
  char *sock_name = NULL;
  ...
  VIR_DEBUG(
      "Choosing remote socket for transport=%s mode=%s driver=%s flags=0x%x",
      remoteDriverTransportTypeToString(transport),
      remoteDriverModeTypeToString(mode), NULLSTR(driver), flags);
  ...
  if (driver) {
    ...
    direct_sock_name =
        remoteGetUNIXSocketHelper(transport, direct_daemon, flags);
  }
  ...
  if (mode == REMOTE_DRIVER_MODE_AUTO) {
    VIR_DEBUG("Resolving mode=auto direct_daemon=%s direct_sock=%s "
              "legacy_daemon=%s legacy_sock=%s",
  ...
  case REMOTE_DRIVER_MODE_DIRECT:
    ...
    if (!direct_sock_name) {
      virReportError(
          VIR_ERR_OPERATION_UNSUPPORTED, "%s",
          _("No URI is provided and cannot identify any listening daemon "
            "socket path to attempt to connect to. Please ensure the expected "
            "daemon sockets are active and/or provide an explicit URI. For "
  ...
  if (flags & REMOTE_DRIVER_OPEN_AUTOSTART) {
    if (!(*daemon_path = virFileFindResourceFull(daemon_name, NULL, NULL,
  ...
  VIR_DEBUG("Chosen UNIX sockname=%s daemon_path=%s with mode=%s", sock_name,
            NULLSTR(*daemon_path), remoteDriverModeTypeToString(mode));
  return sock_name;
}
...
void remoteGetURIDaemonInfo(virURI *uri, remoteDriverTransport transport,
  ...
  *flags = 0;
   ...
   */
  if (uri && uri->path && uri->scheme &&
      (STREQ(uri->path, "/session") || STRPREFIX(uri->scheme, "test+")) &&
      geteuid() > 0) {
    ...
    *flags |= REMOTE_DRIVER_OPEN_USER;
     ...
     */
    if (!uri->server && (transport == REMOTE_DRIVER_TRANSPORT_UNIX) &&
        (!autostart_str || STRNEQ(autostart_str, "0"))) {
  ...
  if (!uri) {
    ...
    if (geteuid() > 0) {
      ...
      *flags |= REMOTE_DRIVER_OPEN_USER;
      if (!autostart_str || STRNEQ(autostart_str, "0"))
        *flags |= REMOTE_DRIVER_OPEN_AUTOSTART;
    }
  }
}
