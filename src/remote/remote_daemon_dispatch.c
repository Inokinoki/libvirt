...
get_nonnull_domain_checkpoint(virDomainPtr dom,
                              remote_nonnull_domain_checkpoint checkpoint);
static virDomainSnapshotPtr
get_nonnull_domain_snapshot(virDomainPtr dom,
...
static int
remoteSerializeDomainDiskErrors(virDomainDiskErrorPtr errors, int nerrors,
                                remote_domain_disk_error **ret_errors_val,
                                u_int *ret_errors_len);

static virConnectPtr remoteGetHypervisorConn(virNetServerClient *client);
static virConnectPtr remoteGetInterfaceConn(virNetServerClient *client);
static virConnectPtr remoteGetNetworkConn(virNetServerClient *client);
static virConnectPtr remoteGetNodeDevConn(virNetServerClient *client);
static virConnectPtr remoteGetNWFilterConn(virNetServerClient *client);
static virConnectPtr remoteGetSecretConn(virNetServerClient *client);
static virConnectPtr remoteGetStorageConn(virNetServerClient *client);
...
/* Prototypes */
static void remoteDispatchObjectEventSend(virNetServerClient *client,
                                          virNetServerProgram *program,
                                          int procnr, xdrproc_t proc,
                                          void *data);

static void remoteEventCallbackFree(void *opaque) {
...
static bool remoteRelayDomainEventCheckACL(virNetServerClient *client,
                                           virConnectPtr conn,
                                           virDomainPtr dom) {
...
static bool remoteRelayNetworkEventCheckACL(virNetServerClient *client,
                                            virConnectPtr conn,
                                            virNetworkPtr net) {
...
static bool remoteRelayStoragePoolEventCheckACL(virNetServerClient *client,
                                                virConnectPtr conn,
...
static bool remoteRelayNodeDeviceEventCheckACL(virNetServerClient *client,
                                               virConnectPtr conn,
...
static bool remoteRelaySecretEventCheckACL(virNetServerClient *client,
                                           virConnectPtr conn,
                                           virSecretPtr secret) {
...
static bool remoteRelayDomainQemuMonitorEventCheckACL(
    virNetServerClient *client, virConnectPtr conn, virDomainPtr dom) {
  g_autofree virDomainDef *def = g_new0(virDomainDef, 1);
  g_autoptr(virIdentity) identity = NULL;
  bool ret = false;
...
static int remoteRelayDomainEventLifecycle(virConnectPtr conn, virDomainPtr dom,
  ...
  if (callback->legacy) {
    remoteDispatchObjectEventSend(
        callback->client, callback->program, REMOTE_PROC_DOMAIN_EVENT_LIFECYCLE,
    ...
    remoteDispatchObjectEventSend(
        callback->client, callback->program,
        REMOTE_PROC_DOMAIN_EVENT_CALLBACK_LIFECYCLE,
        (xdrproc_t)xdr_remote_domain_event_callback_lifecycle_msg, &msg);
  }

  return 0;
}

static int remoteRelayDomainEventReboot(virConnectPtr conn, virDomainPtr dom,
                                        void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  if (callback->legacy) {
    remoteDispatchObjectEventSend(
        callback->client, callback->program, REMOTE_PROC_DOMAIN_EVENT_REBOOT,
        (xdrproc_t)xdr_remote_domain_event_reboot_msg, &data);
  } else {
    remote_domain_event_callback_reboot_msg msg = {callback->callbackID, data};
...
static int remoteRelayDomainEventRTCChange(virConnectPtr conn, virDomainPtr dom,
  ...
  VIR_DEBUG(
      "Relaying domain rtc change event %s %d %lld, callback %d legacy %d",
      dom->name, dom->id, offset, callback->callbackID, callback->legacy);
...
static int remoteRelayDomainEventWatchdog(virConnectPtr conn, virDomainPtr dom,
  ...
  VIR_DEBUG("Relaying domain watchdog event %s %d %d, callback %d", dom->name,
            dom->id, action, callback->callbackID);
  ...
  if (callback->legacy) {
    remoteDispatchObjectEventSend(
        callback->client, callback->program, REMOTE_PROC_DOMAIN_EVENT_WATCHDOG,
...
static int remoteRelayDomainEventIOError(virConnectPtr conn, virDomainPtr dom,
                                         const char *srcPath,
                                         const char *devAlias, int action,
                                         void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  VIR_DEBUG("Relaying domain io error %s %d %s %s %d, callback %d", dom->name,
            dom->id, srcPath, devAlias, action, callback->callbackID);
...
static int remoteRelayDomainEventIOErrorReason(
    virConnectPtr conn, virDomainPtr dom, const char *srcPath,
    const char *devAlias, int action, const char *reason, void *opaque) {
  daemonClientEventCallback *callback = opaque;
...
static int remoteRelayDomainEventGraphics(
    virConnectPtr conn, virDomainPtr dom, int phase,
    virDomainEventGraphicsAddressPtr local,
    virDomainEventGraphicsAddressPtr remote, const char *authScheme,
    virDomainEventGraphicsSubjectPtr subject, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  VIR_DEBUG("Relaying domain graphics event %s %d %d - %d %s %s  - %d %s %s - "
            "%s, callback %d",
            dom->name, dom->id, phase, local->family, local->service,
            local->node, remote->family, remote->service, remote->node,
  ...
  data.subject.subject_val =
      g_new0(remote_domain_event_graphics_identity, data.subject.subject_len);
...
static int remoteRelayDomainEventBlockJob(virConnectPtr conn, virDomainPtr dom,
                                          const char *path, int type,
                                          int status, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  if (callback->legacy) {
    remoteDispatchObjectEventSend(
        callback->client, callback->program, REMOTE_PROC_DOMAIN_EVENT_BLOCK_JOB,
...
static int remoteRelayDomainEventControlError(virConnectPtr conn,
  ...
  VIR_DEBUG("Relaying domain control error %s %d, callback %d", dom->name,
            dom->id, callback->callbackID);
...
static int remoteRelayDomainEventDiskChange(
    virConnectPtr conn, virDomainPtr dom, const char *oldSrcPath,
    const char *newSrcPath, const char *devAlias, int reason, void *opaque) {
  daemonClientEventCallback *callback = opaque;
...
static int remoteRelayDomainEventTrayChange(virConnectPtr conn,
                                            virDomainPtr dom,
  ...
  VIR_DEBUG(
      "Relaying domain %s %d tray change devAlias: %s reason: %d, callback %d",
...
static int remoteRelayDomainEventPMWakeup(virConnectPtr conn, virDomainPtr dom,
  ...
  VIR_DEBUG("Relaying domain %s %d system pmwakeup, callback %d", dom->name,
            dom->id, callback->callbackID);
...
static int remoteRelayDomainEventPMSuspend(virConnectPtr conn, virDomainPtr dom,
                                           int reason, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  VIR_DEBUG("Relaying domain %s %d system pmsuspend, callback %d", dom->name,
            dom->id, callback->callbackID);
  ...
  if (callback->legacy) {
    remoteDispatchObjectEventSend(
        callback->client, callback->program, REMOTE_PROC_DOMAIN_EVENT_PMSUSPEND,
...
static int remoteRelayDomainEventBalloonChange(virConnectPtr conn,
                                               virDomainPtr dom,
...
static int remoteRelayDomainEventPMSuspendDisk(virConnectPtr conn,
                                               virDomainPtr dom, int reason,
                                               void *opaque) {
  daemonClientEventCallback *callback = opaque;
...
static int remoteRelayDomainEventDeviceRemoved(virConnectPtr conn,
                                               virDomainPtr dom,
                                               const char *devAlias,
                                               void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  if (callback->legacy) {
    remoteDispatchObjectEventSend(
        ...
        (xdrproc_t)xdr_remote_domain_event_device_removed_msg, &data);
  } else {
    ...
    remoteDispatchObjectEventSend(
        ...
        (xdrproc_t)xdr_remote_domain_event_callback_device_removed_msg, &msg);
  }

  return 0;
}

static int remoteRelayDomainEventBlockJob2(virConnectPtr conn, virDomainPtr dom,
  ...
  remoteDispatchObjectEventSend(
      callback->client, callback->program, REMOTE_PROC_DOMAIN_EVENT_BLOCK_JOB_2,
      (xdrproc_t)xdr_remote_domain_event_block_job_2_msg, &data);

  return 0;
}

static int remoteRelayDomainEventTunable(virConnectPtr conn, virDomainPtr dom,
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_EVENT_TUNABLE_MAX,
          (struct _virTypedParameterRemote **)&data.params.params_val,
          &data.params.params_len, VIR_TYPED_PARAM_STRING_OKAY) < 0)
    return -1;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_domain_event_callback_tunable_msg, &data);

  return 0;
}

static int remoteRelayDomainEventAgentLifecycle(virConnectPtr conn,
                                                virDomainPtr dom, int state,
                                                int reason, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_domain_event_callback_agent_lifecycle_msg, &data);

  return 0;
}

static int remoteRelayDomainEventDeviceAdded(virConnectPtr conn,
                                             virDomainPtr dom,
                                             const char *devAlias,
                                             void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_domain_event_callback_device_added_msg, &data);

  return 0;
}

static int remoteRelayDomainEventMigrationIteration(virConnectPtr conn,
                                                    virDomainPtr dom,
                                                    int iteration,
                                                    void *opaque) {
  daemonClientEventCallback *callback = opaque;
...
static int remoteRelayDomainEventJobCompleted(virConnectPtr conn,
                                              virDomainPtr dom,
                                              virTypedParameterPtr params,
                                              int nparams, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  /* build return data */
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_JOB_STATS_MAX,
          (struct _virTypedParameterRemote **)&data.params.params_val,
          &data.params.params_len, VIR_TYPED_PARAM_STRING_OKAY) < 0)
    return -1;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_domain_event_callback_job_completed_msg, &data);
  return 0;
}

static int remoteRelayDomainEventDeviceRemovalFailed(virConnectPtr conn,
                                                     virDomainPtr dom,
                                                     const char *devAlias,
                                                     void *opaque) {
  daemonClientEventCallback *callback = opaque;
...
static int remoteRelayDomainEventMetadataChange(virConnectPtr conn,
                                                virDomainPtr dom, int type,
                                                const char *nsuri,
                                                void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_domain_event_callback_metadata_change_msg, &data);

  return 0;
}

static int remoteRelayDomainEventBlockThreshold(
    virConnectPtr conn, virDomainPtr dom, const char *dev, const char *path,
...
static int remoteRelayDomainEventMemoryFailure(virConnectPtr conn,
                                               virDomainPtr dom, int recipient,
                                               int action, unsigned int flags,
                                               void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      callback->client, remoteProgram, REMOTE_PROC_DOMAIN_EVENT_MEMORY_FAILURE,
      (xdrproc_t)xdr_remote_domain_event_memory_failure_msg, &data);

  return 0;
}

static int remoteRelayDomainEventMemoryDeviceSizeChange(virConnectPtr conn,
                                                        virDomainPtr dom,
                                                        const char *alias,
  ...
  remoteDispatchObjectEventSend(
      callback->client, remoteProgram,
      REMOTE_PROC_DOMAIN_EVENT_MEMORY_DEVICE_SIZE_CHANGE,
      (xdrproc_t)xdr_remote_domain_event_memory_device_size_change_msg, &data);
  return 0;
}

static int
remoteRelayDomainEventNICMACChange(virConnectPtr conn, virDomainPtr dom,
                                   const char *alias, const char *oldMAC,
                                   const char *newMAC, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      callback->client, remoteProgram, REMOTE_PROC_DOMAIN_EVENT_NIC_MAC_CHANGE,
      (xdrproc_t)xdr_remote_domain_event_nic_mac_change_msg, &data);
  return 0;
}
...
static int remoteRelayNetworkEventLifecycle(virConnectPtr conn,
  ...
  remoteDispatchObjectEventSend(
      callback->client, callback->program, REMOTE_PROC_NETWORK_EVENT_LIFECYCLE,
      (xdrproc_t)xdr_remote_network_event_lifecycle_msg, &data);

  return 0;
}

static int remoteRelayNetworkEventMetadataChange(virConnectPtr conn,
                                                 virNetworkPtr net, int type,
                                                 const char *nsuri,
                                                 void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  VIR_DEBUG("Relaying network metadata change %s %d %s, callback %d", net->name,
            type, NULLSTR(nsuri), callback->callbackID);
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_network_event_callback_metadata_change_msg, &data);
  return 0;
}
...
static int remoteRelayStoragePoolEventLifecycle(virConnectPtr conn,
                                                virStoragePoolPtr pool,
                                                int event, int detail,
                                                void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_storage_pool_event_lifecycle_msg, &data);

  return 0;
}

static int remoteRelayStoragePoolEventRefresh(virConnectPtr conn,
                                              virStoragePoolPtr pool,
                                              void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_storage_pool_event_refresh_msg, &data);
...
G_STATIC_ASSERT(G_N_ELEMENTS(storageEventCallbacks) ==
                VIR_STORAGE_POOL_EVENT_ID_LAST);

static int remoteRelayNodeDeviceEventLifecycle(virConnectPtr conn,
                                               virNodeDevicePtr dev, int event,
                                               int detail, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_node_device_event_lifecycle_msg, &data);

  return 0;
}

static int remoteRelayNodeDeviceEventUpdate(virConnectPtr conn,
                                            virNodeDevicePtr dev,
                                            void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      callback->client, callback->program, REMOTE_PROC_NODE_DEVICE_EVENT_UPDATE,
      (xdrproc_t)xdr_remote_node_device_event_update_msg, &data);
...
G_STATIC_ASSERT(G_N_ELEMENTS(nodeDeviceEventCallbacks) ==
                VIR_NODE_DEVICE_EVENT_ID_LAST);

static int remoteRelaySecretEventLifecycle(virConnectPtr conn,
                                           virSecretPtr secret, int event,
                                           int detail, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  VIR_DEBUG(
      "Relaying node secretice lifecycle event %d, detail %d, callback %d",
      event, detail, callback->callbackID);
  ...
  remoteDispatchObjectEventSend(
      callback->client, callback->program, REMOTE_PROC_SECRET_EVENT_LIFECYCLE,
      (xdrproc_t)xdr_remote_secret_event_lifecycle_msg, &data);

  return 0;
}

static int remoteRelaySecretEventValueChanged(virConnectPtr conn,
                                              virSecretPtr secret,
                                              void *opaque) {
  daemonClientEventCallback *callback = opaque;
  ...
  remoteDispatchObjectEventSend(
      ...
      (xdrproc_t)xdr_remote_secret_event_value_changed_msg, &data);
...
static void remoteRelayDomainQemuMonitorEvent(
    virConnectPtr conn, virDomainPtr dom, const char *event, long long seconds,
    unsigned int micros, const char *details, void *opaque) {
  daemonClientEventCallback *callback = opaque;
  qemu_domain_monitor_event_msg data = {0};

  if (callback->callbackID < 0 ||
      !remoteRelayDomainQemuMonitorEventCheckACL(callback->client, conn, dom))
    return;

  VIR_DEBUG("Relaying qemu monitor event %s %s, callback %d", event, details,
            callback->callbackID);
  ...
  remoteDispatchObjectEventSend(
      callback->client, callback->program, QEMU_PROC_DOMAIN_MONITOR_EVENT,
      (xdrproc_t)xdr_qemu_domain_monitor_event_msg, &data);
  return;
}

static void remoteRelayConnectionClosedEvent(virConnectPtr conn G_GNUC_UNUSED,
                                             int reason, void *opaque) {
  ...
  remoteDispatchObjectEventSend(
      client, remoteProgram, REMOTE_PROC_CONNECT_EVENT_CONNECTION_CLOSED,
      (xdrproc_t)xdr_remote_connect_event_connection_closed_msg, &msg);
}
...
#define DEREG_CB(conn, eventCallbacks, neventCallbacks, deregFcn, name)        \
  do {                                                                         \
    ...
    for (i = 0; i < neventCallbacks; i++) {                                    \
      ...
      VIR_DEBUG("Deregistering remote %s event relay %d", name, callbackID);   \
...
static void remoteClientFreePrivateCallbacks(struct daemonClientPrivate *priv) {
  ...
  DEREG_CB(priv->networkConn, priv->networkEventCallbacks,
           priv->nnetworkEventCallbacks, virConnectNetworkEventDeregisterAny,
           "network");
  ...
  DEREG_CB(priv->secretConn, priv->secretEventCallbacks,
           priv->nsecretEventCallbacks, virConnectSecretEventDeregisterAny,
           "secret");
  DEREG_CB(priv->conn, priv->qemuEventCallbacks, priv->nqemuEventCallbacks,
...
static int remoteOpenConn(const char *uri, bool readonly, bool preserveIdentity,
                          virConnectPtr *conn) {
...
static virConnectPtr remoteGetHypervisorConn(virNetServerClient *client) {
  ...
  if (!priv->conn) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
                   _("hypervisor connection not open"));
    return NULL;
  }
...
static virConnectPtr remoteGetInterfaceConn(virNetServerClient *client) {
  ...
  if (remoteOpenConn(priv->interfaceURI, priv->readonly, true,
...
static virConnectPtr remoteGetNetworkConn(virNetServerClient *client) {
  ...
  if (remoteOpenConn(priv->networkURI, priv->readonly, true,
                     &priv->networkConn) < 0)
    return NULL;
...
static virConnectPtr remoteGetNodeDevConn(virNetServerClient *client) {
  ...
  if (remoteOpenConn(priv->nodedevURI, priv->readonly, true,
                     &priv->nodedevConn) < 0)
    return NULL;
...
static virConnectPtr remoteGetNWFilterConn(virNetServerClient *client) {
  ...
  if (remoteOpenConn(priv->nwfilterURI, priv->readonly, true,
                     &priv->nwfilterConn) < 0)
    return NULL;
...
static virConnectPtr remoteGetSecretConn(virNetServerClient *client) {
  ...
  if (remoteOpenConn(priv->secretURI, priv->readonly, true, &priv->secretConn) <
      0)
    return NULL;
...
static virConnectPtr remoteGetStorageConn(virNetServerClient *client) {
  ...
  if (remoteOpenConn(priv->storageURI, priv->readonly, true,
                     &priv->storageConn) < 0)
    return NULL;
...
void *remoteClientNew(virNetServerClient *client, void *opaque G_GNUC_UNUSED) {
 ...
 */
static int remoteDispatchProbeURI(bool readonly, char **probeduri) {
  ...
  *probeduri = NULL;
  ...
  if (geteuid() != 0) {
    ...
    if (driver == NULL && remoteProbeSessionDriverFromBinary(&driver) < 0)
      return -1;
...
static int remoteDispatchConnectOpen(virNetServer *server G_GNUC_UNUSED,
                                     virNetServerClient *client,
  ...
  if (remoteOpenConn(name, priv->readonly, preserveIdentity, &priv->conn) < 0)
    goto cleanup;
  ...
  if (STREQ(type, "QEMU") || STREQ(type, "Xen") || STREQ(type, "LXC") ||
      STREQ(type, "VBOX") || STREQ(type, "BHYVE") || STREQ(type, "vz") ||
      STREQ(type, "Parallels") || STREQ(type, "CH") ||
      STREQ(type, "macosvf")) {
...
static int remoteDispatchConnectSetIdentity(
    virNetServer *server G_GNUC_UNUSED, virNetServerClient *client,
    virNetMessage *msg G_GNUC_UNUSED, struct virNetMessageError *rerr,
  ...
  if (virTypedParamsDeserialize(
          ...
          args->params.params_len, REMOTE_CONNECT_IDENTITY_PARAMS_MAX, &params,
          &nparams) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetSchedulerParameters(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_SCHEDULER_PARAMETERS_MAX,
...
static int remoteDispatchDomainGetSchedulerParametersFlags(
  ...
  if (virDomainGetSchedulerParametersFlags(dom, params, &nparams, args->flags) <
      0)
    goto cleanup;
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_SCHEDULER_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainBlockPeek(virNetServer *server G_GNUC_UNUSED,
                                         virNetServerClient *client,
                                         virNetMessage *msg G_GNUC_UNUSED,
                                         struct virNetMessageError *rerr,
  ...
  if (virDomainBlockPeek(dom, path, offset, size, ret->buffer.buffer_val,
                         flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainBlockStatsFlags(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_BLOCK_STATS_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainMemoryPeek(virNetServer *server G_GNUC_UNUSED,
                                          virNetServerClient *client,
                                          virNetMessage *msg G_GNUC_UNUSED,
                                          struct virNetMessageError *rerr,
  ...
  if (virDomainMemoryPeek(dom, offset, size, ret->buffer.buffer_val, flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetVcpuPinInfo(
  ...
  if (args->ncpumaps < 0) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
                   _("ncpumaps must be non-negative"));
    goto cleanup;
  }
  if (args->maplen < 0) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
                   _("maplen must be non-negative"));
    goto cleanup;
  }
  if (args->ncpumaps > REMOTE_VCPUINFO_MAX) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
                   _("ncpumaps > REMOTE_VCPUINFO_MAX"));
    goto cleanup;
  }
  ...
  if ((num = virDomainGetVcpuPinInfo(dom, args->ncpumaps, cpumaps, args->maplen,
                                     args->flags)) < 0)
    goto cleanup;
...
static int remoteDispatchDomainPinEmulator(
  ...
  if (virDomainPinEmulator(dom, (unsigned char *)args->cpumap.cpumap_val,
                           args->cpumap.cpumap_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetEmulatorPinInfo(
  ...
  if ((r = virDomainGetEmulatorPinInfo(dom, cpumaps, args->maplen,
                                       args->flags)) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetVcpus(virNetServer *server G_GNUC_UNUSED,
                                        virNetServerClient *client,
                                        virNetMessage *msg G_GNUC_UNUSED,
                                        struct virNetMessageError *rerr,
  ...
  if (args->maxinfo < 0) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
                   _("maxinfo must be non-negative"));
    goto cleanup;
  }
  if (args->maplen < 0) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
  ...
  if ((info_len = virDomainGetVcpus(dom, info, args->maxinfo, cpumaps,
                                    args->maplen)) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetIOThreadInfo(
  ...
  if (ninfo > REMOTE_IOTHREAD_INFO_MAX) {
    virReportError(VIR_ERR_RPC,
                   _("Too many IOThreads in info: %1$d for limit %2$d"), ninfo,
                   REMOTE_IOTHREAD_INFO_MAX);
    goto cleanup;
  }
...
static int remoteDispatchDomainMigratePrepare(
  ...
  if (virDomainMigratePrepare(conn, &cookie, &cookielen, uri_in, uri_out,
...
static int remoteDispatchDomainMigratePrepare2(
  ...
  if (virDomainMigratePrepare2(conn, &cookie, &cookielen, uri_in, uri_out,
...
static int remoteDispatchDomainGetMemoryParameters(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_MEMORY_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetNumaParameters(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_NUMA_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetBlkioParameters(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_BLKIO_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchNodeGetCPUStats(virNetServer *server G_GNUC_UNUSED,
                                         virNetServerClient *client,
                                         virNetMessage *msg G_GNUC_UNUSED,
                                         struct virNetMessageError *rerr,
...
static int remoteDispatchDomainGetLaunchSecurityInfo(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_LAUNCH_SECURITY_INFO_PARAMS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetPerfEvents(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_PERF_EVENTS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, 0) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetBlockIoTune(
    virNetServer *server G_GNUC_UNUSED, virNetServerClient *client,
    virNetMessage *hdr G_GNUC_UNUSED, struct virNetMessageError *rerr,
  ...
  if (virDomainGetBlockIoTune(dom, args->disk ? *args->disk : NULL, params,
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_BLOCK_IO_TUNE_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchAuthList(virNetServer *server,
                                  virNetServerClient *client,
                                  virNetMessage *msg G_GNUC_UNUSED,
                                  struct virNetMessageError *rerr G_GNUC_UNUSED,
 ...
 */
static int remoteDispatchAuthSaslInit(virNetServer *server G_GNUC_UNUSED,
                                      virNetServerClient *client,
                                      virNetMessage *msg G_GNUC_UNUSED,
                                      struct virNetMessageError *rerr,
  ...
  sasl = virNetSASLSessionNewServer(
      saslCtxt, "libvirt", virNetServerClientLocalAddrStringSASL(client),
  ...
  if (virNetServerClientIsSecure(client))
    ...
    virNetSASLSessionSecProps(sasl, virNetSASLContextGetTCPMinSSF(saslCtxt),
...
authfail:
  ...
  virNetMessageSaveError(rerr);
  PROBE(RPC_SERVER_CLIENT_AUTH_FAIL, "client=%p auth=%d", client,
        REMOTE_AUTH_SASL);
  virObjectUnref(sasl);
  return -1;
}
 ...
 */
static int remoteSASLFinish(virNetServer *server, virNetServerClient *client) {
  ...
  PROBE(RPC_SERVER_CLIENT_AUTH_ALLOW, "client=%p auth=%d identity=%s", client,
        REMOTE_AUTH_SASL, identity);
 ...
 */
static int remoteDispatchAuthSaslStart(virNetServer *server,
                                       virNetServerClient *client,
                                       virNetMessage *msg G_GNUC_UNUSED,
                                       struct virNetMessageError *rerr,
  ...
  VIR_DEBUG("Using SASL mechanism %s. Data %d bytes, nil: %d", args->mech,
            args->data.data_len, args->nil);
  err = virNetSASLSessionServerStart(
      priv->sasl, args->mech,
      /* NB, distinction of NULL vs "" is *critical* in SASL */
      args->nil ? NULL : args->data.data_val, args->data.data_len, &serverout,
      &serveroutlen);
  if (err != VIR_NET_SASL_COMPLETE && err != VIR_NET_SASL_CONTINUE)
    goto authfail;
...
authdeny:
  ...
  PROBE(RPC_SERVER_CLIENT_AUTH_DENY, "client=%p auth=%d identity=%s", client,
        REMOTE_AUTH_SASL, identity);
  goto error;
...
static int remoteDispatchAuthSaslStep(virNetServer *server,
                                      virNetServerClient *client,
                                      virNetMessage *msg G_GNUC_UNUSED,
                                      struct virNetMessageError *rerr,
  ...
  VIR_DEBUG("Step using SASL Data %d bytes, nil: %d", args->data.data_len,
            args->nil);
  err = virNetSASLSessionServerStep(
      priv->sasl,
      /* NB, distinction of NULL vs "" is *critical* in SASL */
      args->nil ? NULL : args->data.data_val, args->data.data_len, &serverout,
      &serveroutlen);
  ...
  if (serveroutlen > REMOTE_AUTH_SASL_DATA_MAX) {
    VIR_ERROR(_("sasl step reply data too long %1$d"), (int)serveroutlen);
    goto authfail;
  }
...
static int remoteDispatchAuthPolkit(virNetServer *server,
                                    virNetServerClient *client,
                                    virNetMessage *msg G_GNUC_UNUSED,
                                    struct virNetMessageError *rerr,
  ...
  VIR_WITH_MUTEX_LOCK_GUARD(&priv->lock) {
    action = virNetServerClientGetReadonly(client) ? "org.libvirt.unix.monitor"
                                                   : "org.libvirt.unix.manage";

    VIR_DEBUG("Start PolicyKit auth %d", virNetServerClientGetFD(client));
    if (virNetServerClientGetAuth(client) !=
    ...
    if (timestamp == 0) {
      VIR_WARN(
          "Failing polkit auth due to missing client (pid=%lld) start time",
          (long long)callerPid);
      goto authfail;
    }

    VIR_INFO("Checking PID %lld running as %d", (long long)callerPid,
             callerUid);

    rv =
        virPolkitCheckAuth(action, callerPid, timestamp, callerUid, NULL, true);
    if (rv == -1)
      goto authfail;
    else if (rv == -2)
      goto authdeny;
    ...
    PROBE(RPC_SERVER_CLIENT_AUTH_ALLOW, "client=%p auth=%d identity=%s", client,
          REMOTE_AUTH_POLKIT, ident);
    VIR_INFO("Policy allowed action %s from pid %lld, uid %d", action,
...
static int remoteDispatchConnectRegisterCloseCallback(
  ...
  if (virConnectRegisterCloseCallback(conn, remoteRelayConnectionClosedEvent,
                                      client, NULL) < 0)
    goto cleanup;
...
static int remoteDispatchConnectDomainEventRegister(
  ...
  ref = callback;
  VIR_APPEND_ELEMENT(priv->domainEventCallbacks, priv->ndomainEventCallbacks,
                     callback);

  if ((callbackID = virConnectDomainEventRegisterAny(
           conn, NULL, VIR_DOMAIN_EVENT_ID_LIFECYCLE,
           VIR_DOMAIN_EVENT_CALLBACK(remoteRelayDomainEventLifecycle), ref,
           remoteEventCallbackFree)) < 0) {
    VIR_SHRINK_N(priv->domainEventCallbacks, priv->ndomainEventCallbacks, 1);
    callback = ref;
    goto cleanup;
  }
...
static int remoteDispatchSecretGetValue(virNetServer *server G_GNUC_UNUSED,
                                        virNetServerClient *client,
                                        virNetMessage *msg G_GNUC_UNUSED,
                                        struct virNetMessageError *rerr,
...
static int remoteDispatchDomainGetState(virNetServer *server G_GNUC_UNUSED,
                                        virNetServerClient *client,
                                        virNetMessage *msg G_GNUC_UNUSED,
                                        struct virNetMessageError *rerr,
...
static int remoteDispatchConnectDomainEventRegisterAny(
  ...
  if (args->eventID > VIR_DOMAIN_EVENT_ID_DEVICE_REMOVED || args->eventID < 0) {
  ...
  if ((callbackID = virConnectDomainEventRegisterAny(
           conn, NULL, args->eventID, domainEventCallbacks[args->eventID], ref,
           remoteEventCallbackFree)) < 0) {
    VIR_SHRINK_N(priv->domainEventCallbacks, priv->ndomainEventCallbacks, 1);
    callback = ref;
    goto cleanup;
...
static int remoteDispatchConnectDomainEventCallbackRegisterAny(
  ...
  if (args->dom && !(dom = get_nonnull_domain(conn, *args->dom)))
    goto cleanup;
  ...
  if ((callbackID = virConnectDomainEventRegisterAny(
           conn, dom, args->eventID, domainEventCallbacks[args->eventID], ref,
           remoteEventCallbackFree)) < 0) {
    VIR_SHRINK_N(priv->domainEventCallbacks, priv->ndomainEventCallbacks, 1);
    callback = ref;
    goto cleanup;
...
static int qemuDispatchDomainMonitorCommand(
  ...
  if (virDomainQemuMonitorCommand(dom, args->cmd, &ret->result, args->flags) <
      0)
    goto cleanup;
...
static int qemuDispatchDomainMonitorCommandWithFiles(
    virNetServer *server G_GNUC_UNUSED, virNetServerClient *client,
    virNetMessage *msg, struct virNetMessageError *rerr,
  ...
  /* return 1 here to let virNetServerProgramDispatchCall know we are passing
   * fds */
  if (noutfiles > 0)
    rv = 1;
  else
...
static int remoteDispatchDomainMigrateBegin3(
  ...
  if (!(xml = virDomainMigrateBegin3(dom, xmlin, &cookieout, &cookieoutlen,
                                     args->flags, dname, args->bandwidth)))
    goto cleanup;
...
static int remoteDispatchDomainMigratePrepare3(
  ...
  if (virDomainMigratePrepare3(conn, args->cookie_in.cookie_in_val,
                               args->cookie_in.cookie_in_len, &cookieout,
                               &cookieoutlen, uri_in, uri_out, args->flags,
                               dname, args->bandwidth, args->dom_xml) < 0)
    goto cleanup;
...
static int remoteDispatchDomainMigratePerform3(
  ...
  if (virDomainMigratePerform3(dom, xmlin, args->cookie_in.cookie_in_val,
                               args->cookie_in.cookie_in_len, &cookieout,
                               &cookieoutlen, dconnuri, uri, args->flags, dname,
                               args->bandwidth) < 0)
    goto cleanup;
...
static int remoteDispatchDomainMigrateFinish3(
  ...
  if (!(dom = virDomainMigrateFinish3(
            conn, args->dname, args->cookie_in.cookie_in_val,
            args->cookie_in.cookie_in_len, &cookieout, &cookieoutlen, dconnuri,
            uri, args->flags, args->cancelled)))
    goto cleanup;
...
static int remoteDispatchDomainMigrateConfirm3(
  ...
  if (virDomainMigrateConfirm3(dom, args->cookie_in.cookie_in_val,
                               args->cookie_in.cookie_in_len, args->flags,
                               args->cancelled) < 0)
    goto cleanup;
...
remoteDispatchDomainOpenGraphics(virNetServer *server G_GNUC_UNUSED,
                                 virNetServerClient *client, virNetMessage *msg,
                                 struct virNetMessageError *rerr,
  ...
  if (virDomainOpenGraphics(dom, args->idx, fd, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainOpenGraphicsFd(
  ...
  if ((fd = virDomainOpenGraphicsFD(dom, args->idx, args->flags)) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetInterfaceParameters(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_INTERFACE_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetCPUStats(
  ...
  percpu_len = virDomainGetCPUStats(dom, params, args->nparams, args->start_cpu,
                                    args->ncpus, args->flags);
  if (percpu_len < 0)
    goto cleanup;
  ...
  if (virTypedParamsSerialize(
          params, args->nparams * args->ncpus, REMOTE_DOMAIN_GET_CPU_STATS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetDiskErrors(
  ...
  if (args->maxerrors > REMOTE_DOMAIN_DISK_ERRORS_MAX) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "%s", _("maxerrors too large"));
    goto cleanup;
  }
  ...
  ret->nerrors = len;
  if (errors &&
      remoteSerializeDomainDiskErrors(errors, len, &ret->errors.errors_val,
...
static int remoteDispatchNodeGetSevInfo(virNetServer *server G_GNUC_UNUSED,
                                        virNetServerClient *client,
                                        virNetMessage *msg G_GNUC_UNUSED,
                                        struct virNetMessageError *rerr,
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_NODE_SEV_INFO_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchNodeGetMemoryParameters(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_NODE_MEMORY_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchNodeGetCPUMap(virNetServer *server G_GNUC_UNUSED,
                                       virNetServerClient *client,
                                       virNetMessage *msg G_GNUC_UNUSED,
                                       struct virNetMessageError *rerr,
...
static int lxcDispatchDomainOpenNamespace(
  ...
  ret = virDomainLxcOpenNamespace(dom, &fdlist, args->flags);
  if (ret < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetJobStats(
  ...
  if (virDomainGetJobStats(dom, &ret->type, &params, &nparams, args->flags) < 0)
    goto cleanup;

  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_JOB_STATS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, VIR_TYPED_PARAM_STRING_OKAY) < 0)
    goto cleanup;
...
static int remoteDispatchDomainMigrateBegin3Params(
  ...
  if (args->params.params_len > REMOTE_DOMAIN_MIGRATE_PARAM_LIST_MAX) {
    virReportError(
        VIR_ERR_RPC, _("Too many migration parameters '%1$d' for limit '%2$d'"),
  ...
  if (virTypedParamsDeserialize(
          (struct _virTypedParameterRemote *)args->params.params_val,
          args->params.params_len, 0, &params, &nparams) < 0)
    goto cleanup;

  if (!(xml = virDomainMigrateBegin3Params(dom, params, nparams, &cookieout,
                                           &cookieoutlen, args->flags)))
    goto cleanup;
...
static int remoteDispatchDomainMigratePrepare3Params(
  ...
  if (virDomainMigratePrepare3Params(conn, params, nparams,
                                     ...
                                     args->cookie_in.cookie_in_len, &cookieout,
                                     &cookieoutlen, uri_out, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainMigratePrepareTunnel3Params(
  ...
  if (virDomainMigratePrepareTunnel3Params(
          conn, st, params, nparams, args->cookie_in.cookie_in_val,
          args->cookie_in.cookie_in_len, &cookieout, &cookieoutlen,
          args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainMigrateFinish3Params(
  ...
  dom = virDomainMigrateFinish3Params(
      conn, params, nparams, args->cookie_in.cookie_in_val,
      args->cookie_in.cookie_in_len, &cookieout, &cookieoutlen, args->flags,
      args->cancelled);
  if (!dom)
    goto cleanup;
...
static int remoteDispatchDomainMigrateConfirm3Params(
  ...
  if (virDomainMigrateConfirm3Params(
          dom, params, nparams, args->cookie_in.cookie_in_val,
          args->cookie_in.cookie_in_len, args->flags, args->cancelled) < 0)
    goto cleanup;
...
static int remoteDispatchConnectGetCPUModelNames(
  ...
  len = virConnectGetCPUModelNames(
      conn, args->arch, args->need_results ? &models : NULL, args->flags);
  if (len < 0)
    goto cleanup;

  if (len > REMOTE_CONNECT_CPU_MODELS_MAX) {
    virReportError(VIR_ERR_RPC,
                   _("Too many CPU models '%1$d' for limit '%2$d'"), len,
...
static int remoteDispatchDomainCreateXMLWithFiles(
  ...
  if ((dom = virDomainCreateXMLWithFiles(conn, args->xml_desc, nfiles, files,
                                         args->flags)) == NULL)
    goto cleanup;
...
static int remoteDispatchDomainCreateWithFiles(
  ...
  if (virDomainCreateWithFiles(dom, nfiles, files, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchConnectNetworkEventRegisterAny(
  ...
  if (args->net && !(net = get_nonnull_network(conn, *args->net)))
    goto cleanup;
  ...
  callback->callbackID = -1;
  ref = callback;
  VIR_APPEND_ELEMENT(priv->networkEventCallbacks, priv->nnetworkEventCallbacks,
                     callback);

  if ((callbackID = virConnectNetworkEventRegisterAny(
           conn, net, args->eventID, networkEventCallbacks[args->eventID], ref,
           remoteEventCallbackFree)) < 0) {
    VIR_SHRINK_N(priv->networkEventCallbacks, priv->nnetworkEventCallbacks, 1);
    callback = ref;
    goto cleanup;
  }
...
static int remoteDispatchConnectStoragePoolEventRegisterAny(
  ...
  if (args->pool && !(pool = get_nonnull_storage_pool(conn, *args->pool)))
    goto cleanup;
  ...
  callback->callbackID = -1;
  ref = callback;
  VIR_APPEND_ELEMENT(priv->storageEventCallbacks, priv->nstorageEventCallbacks,
                     callback);

  if ((callbackID = virConnectStoragePoolEventRegisterAny(
           conn, pool, args->eventID, storageEventCallbacks[args->eventID], ref,
           remoteEventCallbackFree)) < 0) {
    VIR_SHRINK_N(priv->storageEventCallbacks, priv->nstorageEventCallbacks, 1);
    callback = ref;
    goto cleanup;
  }
...
static int remoteDispatchConnectNodeDeviceEventRegisterAny(
  ...
  if (args->dev && !(dev = get_nonnull_node_device(conn, *args->dev)))
    goto cleanup;
  ...
  VIR_APPEND_ELEMENT(priv->nodeDeviceEventCallbacks,
                     priv->nnodeDeviceEventCallbacks, callback);

  if ((callbackID = virConnectNodeDeviceEventRegisterAny(
           conn, dev, args->eventID, nodeDeviceEventCallbacks[args->eventID],
           ref, remoteEventCallbackFree)) < 0) {
...
static int remoteDispatchConnectSecretEventRegisterAny(
  ...
  if (args->secret && !(secret = get_nonnull_secret(conn, *args->secret)))
    goto cleanup;
  ...
  callback->callbackID = -1;
  ref = callback;
  VIR_APPEND_ELEMENT(priv->secretEventCallbacks, priv->nsecretEventCallbacks,
                     callback);

  if ((callbackID = virConnectSecretEventRegisterAny(
           conn, secret, args->eventID, secretEventCallbacks[args->eventID],
           ref, remoteEventCallbackFree)) < 0) {
    VIR_SHRINK_N(priv->secretEventCallbacks, priv->nsecretEventCallbacks, 1);
    callback = ref;
    goto cleanup;
  }
...
static int qemuDispatchConnectDomainMonitorEventRegister(
  ...
  callback->callbackID = -1;
  ref = callback;
  VIR_APPEND_ELEMENT(priv->qemuEventCallbacks, priv->nqemuEventCallbacks,
                     callback);

  if ((callbackID = virConnectDomainQemuMonitorEventRegister(
           conn, dom, event, remoteRelayDomainQemuMonitorEvent, ref,
           remoteEventCallbackFree, args->flags)) < 0) {
    VIR_SHRINK_N(priv->qemuEventCallbacks, priv->nqemuEventCallbacks, 1);
    callback = ref;
    goto cleanup;
  }
...
static int qemuDispatchConnectDomainMonitorEventDeregister(
  ...
  if (virConnectDomainQemuMonitorEventDeregister(conn, args->callbackID) < 0)
    goto cleanup;

  VIR_DELETE_ELEMENT(priv->qemuEventCallbacks, i, priv->nqemuEventCallbacks);
...
static int remoteDispatchDomainGetTime(virNetServer *server G_GNUC_UNUSED,
                                       virNetServerClient *client,
                                       virNetMessage *msg G_GNUC_UNUSED,
                                       struct virNetMessageError *rerr,
...
static int remoteDispatchNodeGetFreePages(virNetServer *server G_GNUC_UNUSED,
                                          virNetServerClient *client,
                                          virNetMessage *msg G_GNUC_UNUSED,
                                          struct virNetMessageError *rerr,
  ...
  /* Allocate return buffer. */
  ret->counts.counts_val =
      g_new0(uint64_t, args->pages.pages_len * args->cellCount);

  if ((len = virNodeGetFreePages(
           conn, args->pages.pages_len, args->pages.pages_val, args->startCell,
           args->cellCount, (unsigned long long *)ret->counts.counts_val,
           args->flags)) <= 0)
    goto cleanup;
...
static int remoteSerializeDHCPLease(remote_network_dhcp_lease *lease_dst,
...
static int remoteDispatchNetworkGetDHCPLeases(
  ...
  if ((nleases = virNetworkGetDHCPLeases(net, args->mac ? *args->mac : NULL,
...
static int remoteDispatchConnectGetAllDomainStats(
  ...
  if (args->doms.doms_len) {
    ...
    if ((nrecords = virDomainListGetStats(doms, args->stats, &retStats,
                                          args->flags)) < 0)
      goto cleanup;
  } else {
    if ((nrecords = virConnectGetAllDomainStats(conn, args->stats, &retStats,
                                                args->flags)) < 0)
      goto cleanup;
  }
  ...
  if (nrecords) {
    if (nrecords > REMOTE_DOMAIN_LIST_MAX) {
      virReportError(VIR_ERR_INTERNAL_ERROR,
                     _("Number of domain stats records is %1$d, which exceeds "
                       "max limit: %2$d"),
...
static int remoteDispatchNodeAllocPages(virNetServer *server G_GNUC_UNUSED,
                                        virNetServerClient *client,
                                        virNetMessage *msg G_GNUC_UNUSED,
                                        struct virNetMessageError *rerr,
  ...
  if ((len = virNodeAllocPages(
           conn, args->pageSizes.pageSizes_len, args->pageSizes.pageSizes_val,
           (unsigned long long *)args->pageCounts.pageCounts_val,
           args->startCell, args->cellCount, args->flags)) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetFSInfo(virNetServer *server G_GNUC_UNUSED,
                                         virNetServerClient *client,
                                         virNetMessage *msg G_GNUC_UNUSED,
                                         struct virNetMessageError *rerr,
  ...
  if (ret->ifaces.ifaces_val) {
    for (i = 0; i < ifaces_count; i++) {
      ...
      for (j = 0; j < iface_ret->addrs.addrs_len; j++) {
        remote_domain_ip_addr *ip_addr = &(iface_ret->addrs.addrs_val[j]);
...
static int remoteDispatchDomainInterfaceAddresses(
  ...
  if ((ifaces_count = virDomainInterfaceAddresses(dom, &ifaces, args->source,
                                                  args->flags)) < 0)
    goto cleanup;
...
static int remoteDispatchNetworkPortGetParameters(
  ...
  if (virTypedParamsSerialize(
          params, nparams, REMOTE_NETWORK_PORT_PARAMETERS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, args->flags) < 0)
    goto cleanup;
...
static int remoteDispatchDomainGetGuestInfo(
  ...
  if (virDomainGetGuestInfo(dom, args->types, &params, &nparams, args->flags) <
      0)
    goto cleanup;

  if (virTypedParamsSerialize(
          params, nparams, REMOTE_DOMAIN_GUEST_INFO_PARAMS_MAX,
          (struct _virTypedParameterRemote **)&ret->params.params_val,
          &ret->params.params_len, VIR_TYPED_PARAM_STRING_OKAY) < 0)
    goto cleanup;
...
static int remoteDispatchDomainAuthorizedSshKeysGet(
  ...
  if ((nkeys = virDomainAuthorizedSSHKeysGet(dom, args->user, &keys,
                                             args->flags)) < 0)
    goto cleanup;
