/*
 * macosvf_driver.c: core driver methods for managing macOS Virtualization.Framework guests
 *
 * Copyright (C) 2025
 *
...
static int
macosvfAutostartDomain(virDomainObj *vm,
...
static int
macosvfSaveDomainConfig(macosvfConn *driver,
                        virDomainObj *vm)
{
    ...
    if (virFileWriteStr(configFile, xml, 0600) < 0) {
        virReportSystemError(errno,
                             _("Cannot write config file %1$s"),
                             configFile);
        return -1;
    }
...
static int
macosvfDeleteDomainConfig(macosvfConn *driver,
                          virDomainObj *vm)
{
    g_autofree char *configFile = NULL;
    ...
    if (virFileExists(configFile)) {
        if (unlink(configFile) < 0) {
            virReportSystemError(errno,
                                 _("Cannot delete config file %1$s"),
                                 configFile);
            return -1;
        }
    }
...
static const char *
macosvfConnectGetType(virConnectPtr conn G_GNUC_UNUSED)
{
    return "macosvf";
}

static int
macosvfConnectGetVersion(virConnectPtr conn G_GNUC_UNUSED,
                         unsigned long *hvVer)
{
...
static char *
macosvfConnectGetHostname(virConnectPtr conn G_GNUC_UNUSED)
{
    char *hostname;
...
static int
macosvfNodeGetInfo(virConnectPtr conn,
...
static char *
macosvfConnectGetCapabilities(virConnectPtr conn)
{
...
static int
macosvfConnectListDomains(virConnectPtr conn,
                          int *ids,
                          int maxids)
{
    macosvfConn *privconn = conn->privateData;
...
static int
macosvfConnectNumOfDomains(virConnectPtr conn)
{
    macosvfConn *privconn = conn->privateData;
    int n;
...
static int
macosvfConnectListAllDomains(virConnectPtr conn,
...
static virDomainPtr
macosvfDomainLookupByID(virConnectPtr conn,
                         int id)
{
    macosvfConn *privconn = conn->privateData;
...
static virDomainPtr
macosvfDomainLookupByUUID(virConnectPtr conn,
...
static virDomainPtr
macosvfDomainLookupByName(virConnectPtr conn,
                           const char *name)
{
    macosvfConn *privconn = conn->privateData;
    virDomainObj *vm = NULL;
...
static int
macosvfConnectGetMaxVcpus(virConnectPtr conn G_GNUC_UNUSED,
...
static int
macosvfDomainGetInfo(virDomainPtr dom,
                      virDomainInfoPtr info)
{
    ...
    if (priv && priv->vm) {
        ...
        macosvfVMGetMemoryStats((macosvfVMObject *)priv->vm, &memoryUsed);
    }

    info->cpuTime = cpuTime;
    info->memory = memoryUsed > 0 ? memoryUsed : virDomainDefGetMemoryTotal(vm->def);
...
static int
macosvfDomainGetState(virDomainPtr dom,
                       int *state,
                       int *reason,
                       unsigned int flags)
{
...
static char *
macosvfDomainGetXMLDesc(virDomainPtr dom,
                         unsigned int flags)
{
    virDomainObj *vm;
    char *ret = NULL;

    virCheckFlags(VIR_DOMAIN_XML_INACTIVE |
                  VIR_DOMAIN_XML_MIGRATABLE, NULL);
...
static int
macosvfDomainMemoryStats(virDomainPtr dom,
...
static char *
macosvfDomainGetSchedulerType(virDomainPtr dom G_GNUC_UNUSED,
                              int *nparams)
{
    ...
    return g_strdup("macosvf");
}

static int
macosvfDomainGetSchedulerParametersFlags(virDomainPtr dom,
...
static int
macosvfDomainGetSchedulerParameters(virDomainPtr dom,
                                   virTypedParameterPtr params,
                                   int *nparams)
{
...
static int
macosvfDomainSetSchedulerParametersFlags(virDomainPtr dom,
                                        virTypedParameterPtr params,
                                        int nparams,
                                        unsigned int flags)
{
    ...
    for (i = 0; i < nparams; i++) {
        if (STREQ(params[i].field, VIR_DOMAIN_SCHEDULER_CPU_SHARES)) {
            VIR_DEBUG("Ignoring CPU shares request: %llu",
                      params[i].value.ul);
...
static int
macosvfDomainSetSchedulerParameters(virDomainPtr dom,
                                   virTypedParameterPtr params,
                                   int nparams)
{
...
static int
macosvfDomainSetBlockIoTune(virDomainPtr dom,
                            const char *path,
                            virTypedParameterPtr params,
                            int nparams,
                            unsigned int flags)
{
    ...
    VIR_DEBUG("Ignoring I/O tune request for disk %s (framework manages I/O internally)",
              path);
...
static int
macosvfDomainGetBlockIoTune(virDomainPtr dom,
                            const char *path,
                            virTypedParameterPtr params,
                            int *nparams,
                            unsigned int flags)
{
    ...
    i = 0;
    if (i < *nparams) {
        if (virTypedParameterAssign(&params[i], VIR_DOMAIN_BLOCK_IOTUNE_TOTAL_BYTES_SEC,
    ...
    if (i < *nparams) {
        if (virTypedParameterAssign(&params[i], VIR_DOMAIN_BLOCK_IOTUNE_READ_BYTES_SEC,
                                    VIR_TYPED_PARAM_ULLONG, 0) < 0)
            goto cleanup;
        i++;
    }
    ...
    if (i < *nparams) {
        if (virTypedParameterAssign(&params[i], VIR_DOMAIN_BLOCK_IOTUNE_WRITE_BYTES_SEC,
                                    VIR_TYPED_PARAM_ULLONG, 0) < 0)
            goto cleanup;
        i++;
    }
    ...
    if (i < *nparams) {
        if (virTypedParameterAssign(&params[i], VIR_DOMAIN_BLOCK_IOTUNE_TOTAL_IOPS_SEC,
                                    VIR_TYPED_PARAM_ULLONG, 0) < 0)
            goto cleanup;
        i++;
    }
    ...
    if (i < *nparams) {
        if (virTypedParameterAssign(&params[i], VIR_DOMAIN_BLOCK_IOTUNE_READ_IOPS_SEC,
                                    VIR_TYPED_PARAM_ULLONG, 0) < 0)
            goto cleanup;
        i++;
    }
    ...
    if (i < *nparams) {
        if (virTypedParameterAssign(&params[i], VIR_DOMAIN_BLOCK_IOTUNE_WRITE_IOPS_SEC,
                                    VIR_TYPED_PARAM_ULLONG, 0) < 0)
            goto cleanup;
        i++;
    }
...
static int
macosvfDomainSetMemoryParameters(virDomainPtr dom,
                                virTypedParameterPtr params,
                                int nparams,
                                unsigned int flags)
{
        ...
        if (STREQ(params[i].field, VIR_DOMAIN_MEMORY_HARD_LIMIT)) {
            ...
            if (hard_limit != 0 && hard_limit < def->mem.cur_balloon) {
                virReportError(VIR_ERR_INVALID_ARG, "%s",
                               _("memory hard_limit tunable value must be higher than current memory"));
                goto cleanup;
            }
...
static int
macosvfDomainGetMemoryParameters(virDomainPtr dom,
                                virTypedParameterPtr params,
                                int *nparams,
                                unsigned int flags)
{
    ...
    if (i < *nparams) {
        if (virTypedParameterAssign(&params[i], VIR_DOMAIN_MEMORY_MIN_GUARANTEE,
                                    VIR_TYPED_PARAM_ULLONG, def->mem.min_guarantee) < 0)
            goto cleanup;
        i++;
    }
...
static int
macosvfDomainSetNumaParameters(virDomainPtr dom,
                              virTypedParameterPtr params,
                              int nparams,
                              unsigned int flags)
{
    ...
    if (mode != -1) {
        ...
        if (flags & VIR_DOMAIN_AFFECT_CONFIG && persistentDef) {
            ...
            VIR_DEBUG("Setting NUMA mode to %d (framework manages NUMA internally)", mode);
        }
    }
    ...
    if (nodeset) {
        ...
        VIR_DEBUG("Setting NUMA nodeset to %s (framework manages NUMA internally)", nodeset);
    }
...
static int
macosvfDomainGetNumaParameters(virDomainPtr dom,
                              virTypedParameterPtr params,
                              int *nparams,
                              unsigned int flags)
{
...
static int
macosvfDomainSetInterfaceParameters(virDomainPtr dom,
                                    const char *device,
                                    virTypedParameterPtr params,
                                    int nparams,
                                    unsigned int flags)
{
    ...
    if (!net) {
        virReportError(VIR_ERR_INVALID_ARG,
                       _("invalid interface name: %1$s"), device);
        goto cleanup;
    }
     ...
     * so bandwidth limits are accepted but not enforced. This provides compatibility
     * with management tools that expect to set these parameters. */
    if (flags & VIR_DOMAIN_AFFECT_LIVE) {
        ...
        VIR_DEBUG("Setting interface bandwidth for %s (framework manages network internally)", device);
    }
    ...
    if (flags & VIR_DOMAIN_AFFECT_CONFIG && persistentDef) {
        ...
        if (!persistentNet) {
            virReportError(VIR_ERR_INVALID_ARG,
                           _("invalid interface name in persistent config: %1$s"), device);
            goto cleanup;
        }
...
static int
macosvfDomainGetInterfaceParameters(virDomainPtr dom,
                                    const char *device,
                                    virTypedParameterPtr params,
                                    int *nparams,
                                    unsigned int flags)
...
static int
macosvfDomainGetControlInfo(virDomainPtr dom,
...
static int
macosvfDomainBlockStats(virDomainPtr dom,
                       const char *path,
                       virDomainBlockStatsPtr stats)
{
    ...
    /* macOS Virtualization.Framework doesn't provide per-block statistics */
    /* We return 0 for all counters as they're not available */
...
static int
macosvfDomainInterfaceStats(virDomainPtr dom,
                            const char *path,
                            virDomainInterfaceStatsPtr stats)
{
    ...
    /* macOS Virtualization.Framework doesn't provide per-interface statistics */
    /* We return 0 for all counters as they're not available */
...
static int
macosvfDomainShutdownFlags(virDomainPtr dom,
                            unsigned int flags)
{
    virDomainObj *vm;
    macosvfDomainObjPrivate *priv;
...
static int
macosvfDomainShutdown(virDomainPtr dom)
{
    return macosvfDomainShutdownFlags(dom, 0);
}

static int
macosvfDomainDestroyFlags(virDomainPtr dom,
                           unsigned int flags)
{
    virDomainObj *vm;
    macosvfDomainObjPrivate *priv;
...
static int
macosvfDomainDestroy(virDomainPtr dom)
{
    return macosvfDomainDestroyFlags(dom, 0);
}

static int
macosvfDomainSuspend(virDomainPtr dom)
{
    virDomainObj *vm;
    macosvfDomainObjPrivate *priv;
    int ret = -1;
...
static int
macosvfDomainResume(virDomainPtr dom)
{
    virDomainObj *vm;
    macosvfDomainObjPrivate *priv;
    int ret = -1;
...
static virDomainPtr
macosvfDomainCreateXML(virConnectPtr conn,
                       const char *xml,
                       unsigned int flags)
{
    macosvfConn *privconn = conn->privateData;
    ...
    if (macosvfVMCreate(vm->def, &vmobj) < 0) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       "%s", _("Failed to create VM object"));
        goto cleanup;
    }
...
static int
macosvfDomainCreate(virDomainPtr dom)
{
    virDomainObj *vm;
    macosvfDomainObjPrivate *priv;
    macosvfVMObject *vmobj = NULL;
...
static char *
macosvfDomainGetOSType(virDomainPtr dom)
{
    virDomainObj *vm;
    char *ret = NULL;
...
macosvfDomainGetMaxMemory(virDomainPtr dom)
{
    virDomainObj *vm;
...
static int
macosvfDomainSetMemoryFlags(virDomainPtr dom G_GNUC_UNUSED,
...
static int
macosvfDomainSetMemory(virDomainPtr dom, unsigned long memory)
{
    return macosvfDomainSetMemoryFlags(dom, memory, 0);
}

static int
macosvfConnectListDefinedDomains(virConnectPtr conn,
                                  char **const names,
                                  int maxnames)
{
    macosvfConn *privconn = conn->privateData;
...
static int
macosvfConnectNumOfDefinedDomains(virConnectPtr conn)
{
    macosvfConn *privconn = conn->privateData;
    int n;
...
static virDomainPtr
macosvfDomainDefineXMLFlags(virConnectPtr conn,
                             const char *xml,
                             unsigned int flags)
{
    macosvfConn *privconn = conn->privateData;
...
static virDomainPtr
macosvfDomainDefineXML(virConnectPtr conn,
                        const char *xml)
{
...
static int
macosvfDomainUndefineFlags(virDomainPtr dom,
                            unsigned int flags)
{
    virDomainObj *vm;
    ...
    if (virDomainObjIsActive(vm)) {
        virReportError(VIR_ERR_OPERATION_INVALID,
                       _("cannot undefine active domain '%1$s'"),
                       vm->def->name);
        goto cleanup;
    }
...
static int
macosvfDomainUndefine(virDomainPtr dom)
{
...
macosvfConnectOpen(virConnectPtr conn,
                   ...
                   unsigned int flags)
{
    ...
    if (conn->uri &&
        STRNEQ(conn->uri->scheme, "macosvf") &&
    ...
    if (macosvf_driver == NULL) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       "%s", _("macosvf state driver is not active"));
...
static int
macosvfConnectClose(virConnectPtr conn)
{
    if (!conn->privateData)
        return 0;
...
macosvfStateInitialize(bool privileged,
                       const char *root,
                       ...
                       void *opaque G_GNUC_UNUSED)
{
    ...
    if (!(driver->xmlopt = virDomainXMLOptionNew(NULL,
    ...
    if (driver->config->autoStart) {
        virDomainObjListForEach(driver->domains, false, macosvfAutostartDomain, NULL);
    }
...
static virHypervisorDriver macosvfHypervisorDriver = {
    ...
    .domainGetSchedulerParameters = macosvfDomainGetSchedulerParameters, /* 10.10.0 */
    .domainGetSchedulerParametersFlags = macosvfDomainGetSchedulerParametersFlags, /* 10.10.0 */
    .domainSetSchedulerParameters = macosvfDomainSetSchedulerParameters, /* 10.10.0 */
    .domainSetSchedulerParametersFlags = macosvfDomainSetSchedulerParametersFlags, /* 10.10.0 */
    ...
    .domainSetInterfaceParameters = macosvfDomainSetInterfaceParameters, /* 10.10.0 */
    .domainGetInterfaceParameters = macosvfDomainGetInterfaceParameters, /* 10.10.0 */
    .connectListDefinedDomains = macosvfConnectListDefinedDomains, /* 10.10.0 */
    .connectNumOfDefinedDomains = macosvfConnectNumOfDefinedDomains, /* 10.10.0 */
