...
@interface VZManager () {
    ...
    NSMutableDictionary<NSString *, VZVirtualMachineConfiguration *> *_configurations;
...
- (void)createVMWithId:(NSString *)vmId
    ...
    @autoreleasepool {
        ...
        VZVirtualMachineConfiguration *vzConfig = [[VZVirtualMachineConfiguration alloc] init];
        ...
        vzConfig.memorySize = (memorySize) ? [memorySize longLongValue] : (1024 * 1024 * 1024); /* 1GB default */
        ...
        if (kernelPath) {
            ...
            NSURL *storeURL = [NSURL fileURLWithPath:[NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"%@_nvram.plist", vmId]]];
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:storeURL.path]) {
                ...
                NSLog(@"VZManager: Using existing EFI variable store at %@", storeURL.path);
            }

            /* If that failed or file didn't exist, create new one */
            if (!variableStore) {
                variableStore = [[VZEFIVariableStore alloc] initCreatingVariableStoreAtURL:storeURL options:0 error:&storeError];
            }
            ...
            if (variableStore) {
                ...
                NSLog(@"VZManager: Failed to create EFI variable store: %@", storeError);
                if (completion) {
                    NSError *error = [NSError errorWithDomain:@"VZManager"
                                                         code:4
                                                     userInfo:@{NSLocalizedDescriptionKey:
                                                                 [NSString stringWithFormat:@"Failed to create EFI variable store: %@", storeError.localizedDescription]}];
                    completion(NO, error);
                }
                return;
            }
         ...
         */
            ...
            if (readHandle && writeHandle) {
                ...
                NSLog(@"VZManager: Failed to open PTY file handles: %s", strerror(errno));
            }
        }
        ...
        if (diskPaths && diskPaths.count > 0) {
            ...
            for (NSUInteger i = 0; i < diskPaths.count; i++) {
                ...
                if (diskAttachment) {
                    ...
                    NSLog(@"VZManager: Successfully created disk attachment for %@", diskPath);
                } else {
                    NSLog(@"VZManager: Failed to create disk attachment for %@: %@", diskPath, attachmentError);
                    /* Fail the VM creation if we can't attach the disk */
                    if (completion) {
                        NSError *error = [NSError errorWithDomain:@"VZManager"
                                                             code:3
                                                         userInfo:@{NSLocalizedDescriptionKey:
                                                                     [NSString stringWithFormat:@"Failed to attach disk: %@", attachmentError.localizedDescription]}];
                        completion(NO, error);
                    }
                    return;
                }
            ...
            if (storageDevices.count > 0) {
                ...
                NSLog(@"VZManager: Added %lu storage devices", (unsigned long)storageDevices.count);
            }
        } else {
        ...
        if (![vzConfig validateWithError:&validationError]) {
        ...
        /* Create the VM */
        VZVirtualMachine *vm = [[VZVirtualMachine alloc] initWithConfiguration:vzConfig];
        _virtualMachines[vmId] = vm;

        NSLog(@"VZManager: VM %@ created successfully", vmId);
...
- (void)pauseVMWithId:(NSString *)vmId
    ...
    if (@available(macOS 13.0, *)) {
        ...
        NSError *error = [NSError errorWithDomain:@"VZManager"
                                             code:2
                                         userInfo:@{NSLocalizedDescriptionKey: @"Pause not supported on this macOS version"}];
        if (completion) {
            completion(NO, error);
        }
    }
...
- (void)resumeVMWithId:(NSString *)vmId
    ...
    if (@available(macOS 13.0, *)) {
        [vm resumeWithCompletionHandler:^(NSError * _Nullable error) {
            NSLog(@"VZManager: VM %@ resume completed: %@", vmId, error ?: @"success");
            if (completion) {
                completion(error == nil, error);
            }
        }];
        ...
        NSError *error = [NSError errorWithDomain:@"VZManager"
                                             code:2
                                         userInfo:@{NSLocalizedDescriptionKey: @"Resume not supported on this macOS version"}];
        if (completion) {
            completion(NO, error);
        }
    }
...
- (NSString *)stateToString:(VZManagerVMState)state {
    switch (state) {
        case VZManagerVMStateStopped: return @"stopped";
        case VZManagerVMStateRunning: return @"running";
        case VZManagerVMStatePaused:  return @"paused";
        case VZManagerVMStateError:   return @"error";
        case VZManagerVMStateStarting: return @"starting";
        case VZManagerVMStateStopping: return @"stopping";
