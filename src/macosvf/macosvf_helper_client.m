...
- (void)getConsolePathForVMWithId:(NSString *)vmId
                            reply:
                                (void (^)(NSString *path, NSError *error))reply;
...
int macosvfHelperClientInit(macosvfHelperClient *client) {
  ...
  @autoreleasepool {
    ...
    VIR_INFO("Creating XPC connection to Mach service: %s",
             [XPC_MACH_PORT_NAME UTF8String]);

    /* Create XPC connection - try privileged option first */
    client->connection = [[NSXPCConnection alloc]
        initWithMachServiceName:XPC_MACH_PORT_NAME
    ...
    if (!client->connection) {
      ...
      client->connection =
          [[NSXPCConnection alloc] initWithMachServiceName:XPC_MACH_PORT_NAME
                                                   options:0];
    }
    ...
    [client->connection
        setRemoteObjectInterface:[NSXPCInterface
...
int macosvfHelperCreateVM(macosvfHelperClient *client, const char *vmId,
  ...
  @autoreleasepool {
    id<VZXPCProtocol> remote =
        [client->connection remoteObjectProxyWithErrorHandler:^(NSError *err) {
          VIR_ERROR("XPC connection error: %s",
    ...
    if (config->kernelPath) {
      [configDict setObject:[NSString stringWithUTF8String:config->kernelPath]
                     forKey:@"kernelPath"];
    }
    if (config->initrdPath) {
      [configDict setObject:[NSString stringWithUTF8String:config->initrdPath]
                     forKey:@"initrdPath"];
    }
    if (config->cmdline) {
      [configDict setObject:[NSString stringWithUTF8String:config->cmdline]
                     forKey:@"cmdline"];
    }
    if (config->ptyPath) {
      [configDict setObject:[NSString stringWithUTF8String:config->ptyPath]
                     forKey:@"ptyPath"];
    }
    ...
    [configDict setObject:@(config->enableDisplay ? 1 : 0)
                   forKey:@"enableDisplay"];

    /* Add port forwarding rules */
    if (config->numPortForwards > 0) {
      NSMutableArray *fwArray = [NSMutableArray array];
      for (int i = 0;
           i < config->numPortForwards && i < MACOSVF_MAX_PORT_FORWARDS; i++) {
        NSMutableDictionary *fwDict = [NSMutableDictionary dictionary];
        [fwDict setObject:@(config->portForwards[i].hostPort)
                   forKey:@"hostPort"];
        [fwDict setObject:@(config->portForwards[i].guestPort)
                   forKey:@"guestPort"];
        [fwDict setObject:@(config->portForwards[i].protocol ? YES : NO)
                   forKey:@"isUDP"];
        [fwArray addObject:fwDict];
      }
      [configDict setObject:fwArray forKey:@"portForwards"];
      NSLog(@"macosvf_helper_client: Adding %d port forwards",
    ...
    if (config->numDisks > 0) {
      ...
      for (int i = 0; i < config->numDisks && i < MACOSVF_MAX_DISKS; i++) {
        if (config->diskPaths[i]) {
          [diskArray
              addObject:[NSString stringWithUTF8String:config->diskPaths[i]]];
        }
      }
  ...
  if (error) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "Failed to create VM: %s",
...
int macosvfHelperStartVM(macosvfHelperClient *client, const char *vmId) {
  ...
  if (error) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "Failed to start VM: %s",
                   [[error localizedDescription] UTF8String]);
    return -1;
  }
...
int macosvfHelperStopVM(macosvfHelperClient *client, const char *vmId) {
  ...
  if (error) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "Failed to stop VM: %s",
                   [[error localizedDescription] UTF8String]);
    return -1;
  }
...
int macosvfHelperPauseVM(macosvfHelperClient *client, const char *vmId) {
  ...
  if (error) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "Failed to pause VM: %s",
                   [[error localizedDescription] UTF8String]);
    return -1;
  }
...
int macosvfHelperResumeVM(macosvfHelperClient *client, const char *vmId) {
  ...
  if (error) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "Failed to resume VM: %s",
                   [[error localizedDescription] UTF8String]);
    return -1;
  }
...
int macosvfHelperDestroyVM(macosvfHelperClient *client, const char *vmId) {
  ...
  if (error) {
    virReportError(VIR_ERR_INTERNAL_ERROR, "Failed to destroy VM: %s",
                   [[error localizedDescription] UTF8String]);
    return -1;
  }
